...
# 根目录下的 hotels/REBORN.m3u (假设 py 脚本在 py/ 文件夹，或者根据实际调整)
# 这里使用绝对路径定位，确保从仓库根目录执行时也能找到
BASE_PATH = os.path.dirname(CURRENT_DIR) # 假设脚本在 py/，BASE_PATH 就是项目根目录
...
def main():
    ...
    if not os.path.exists(SOURCE_M3U):
        ...
        # 调试信息：列出当前能看到的 hotels 目录内容
        hotels_dir = os.path.join(BASE_PATH, "hotels")
        if os.path.exists(hotels_dir):
            print(f"当前 hotels 目录下的文件: {os.listdir(hotels_dir)}")
        return
    ...
    print(f"📡 共有 {len(groups)} 个服务器，共抽取 {len(tasks)} 个频道进行采样压测...")
    ...
    print(f"\n✨ 测速任务结束！")
    print(f"📊 报告文本: {OUTPUT_TXT}")
    print(f"📊 报告 JSON: {OUTPUT_JSON}")
